...
var _ = Describe("fpdf_attachment", func() {
	...
	Context("no document is given", func() {
		...
		It("returns an error when FPDFDoc_GetAttachment is called", func() {
			FPDFDoc_GetAttachment, err := PdfiumInstance.FPDFDoc_GetAttachment(&requests.FPDFDoc_GetAttachment{})
			Expect(err).To(MatchError("document not given"))
			Expect(FPDFDoc_GetAttachment).To(BeNil())
		})

		It("returns an error when FPDFDoc_DeleteAttachment is called", func() {
			FPDFDoc_DeleteAttachment, err := PdfiumInstance.FPDFDoc_DeleteAttachment(&requests.FPDFDoc_DeleteAttachment{})
			Expect(err).To(MatchError("document not given"))
	...
	Context("no attachment is given", func() {
		...
		It("returns an error when FPDFAttachment_HasKey is called", func() {
			FPDFAttachment_HasKey, err := PdfiumInstance.FPDFAttachment_HasKey(&requests.FPDFAttachment_HasKey{})
			Expect(err).To(MatchError("attachment not given"))
			Expect(FPDFAttachment_HasKey).To(BeNil())
		})

		It("returns an error when FPDFAttachment_SetStringValue is called", func() {
			FPDFAttachment_SetStringValue, err := PdfiumInstance.FPDFAttachment_SetStringValue(&requests.FPDFAttachment_SetStringValue{})
			Expect(err).To(MatchError("attachment not given"))
			Expect(FPDFAttachment_SetStringValue).To(BeNil())
		})

		It("returns an error when FPDFAttachment_GetStringValue is called", func() {
			FPDFAttachment_GetStringValue, err := PdfiumInstance.FPDFAttachment_GetStringValue(&requests.FPDFAttachment_GetStringValue{})
			Expect(err).To(MatchError("attachment not given"))
	...
	Context("a PDF file with attachments", func() {
		...
		BeforeEach(func() {
			pdfData, err := ioutil.ReadFile(TestDataPath + "/testdata/embedded_attachments.pdf")
			Expect(err).To(BeNil())
		...
		It("returns the correct attachments", func() {
			GetAttachments, err := PdfiumInstance.GetAttachments(&requests.GetAttachments{
				Document: doc,
			})
			Expect(err).To(BeNil())
		...
		When("the first attachment has been loaded", func() {
			...
			It("returns the right value type for key Size", func() {
				FPDFAttachment_GetValueType, err := PdfiumInstance.FPDFAttachment_GetValueType(&requests.FPDFAttachment_GetValueType{
					Attachment: attachment,
					Key:        "Size",
				})
				Expect(err).To(BeNil())
